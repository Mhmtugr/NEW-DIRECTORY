 ...
 * @returns {Promise<Object>} Çıkış işlemi sonucu
 */
...
async function updateProfile(userData) {
  try {
    ...
    if (!firebase || !firebase.firestore) {
      if (authState.isDemo) {
        showToast('Demo modunda profil güncelleme simüle ediliyor...', 'info');
        // Demo modunda güncelleme simüle ediliyor
        setTimeout(() => {
          showToast('Profil bilgileriniz güncellendi (Demo)', 'success');
        }, 1000);
        
        authState.isProcessing = false;
        return { success: true, demo: true, updatedData: userData };
      } else {
 ...
 * @returns {Promise<Object>} - Şifre değiştirme işlemi sonucu
 */
async function changePassword(currentPassword, newPassword) {
  try {
    ...
    if (!firebase || !firebase.auth) {
      if (authState.isDemo) {
        showToast('Demo modunda şifre değiştirme simüle ediliyor...', 'info');
        // Demo modunda değiştirme simüle ediliyor
        setTimeout(() => {
          showToast('Şifreniz başarıyla değiştirildi (Demo)', 'success');
        }, 1000);
...
function checkUserRole(requiredRole) {
  ...
  const userRoleScore = authState.userRoles[userRole] || 0;
  const requiredRoleScore = authState.userRoles[requiredRole] || 100;
...
/**
 * Google ile giriş
 * @returns {Promise<Object>} - Google ile giriş işlemi sonucu
 */
async function loginWithGoogle() {
  try {
    ...
    if (!firebase || !firebase.auth) {
      if (authState.isDemo) {
        showToast('Demo modunda Google giriş simüle ediliyor...', 'info');
        // Demo modunda giriş simüle ediliyor
        setTimeout(() => {
          demoLogin();
        }, 1000);
        ...
        showToast('Firebase bağlantısı bulunamadı, demo moda geçiliyor', 'warning');
        _enableDemoModeWithLoginPrompt();
 ...
 * @returns {Promise<Object>} - Hesap silme işlemi sonucu
 */
async function deleteAccount(password = null) {
  try {
    ...
    if (!firebase || !firebase.auth) {
      if (authState.isDemo) {
        showToast('Demo modunda hesap silme işlemi simüle ediliyor...', 'info');
        // Demo modunda işlem simüle ediliyor
        setTimeout(() => {
          showToast('Hesabınız silindi (Demo)', 'success');
          showLogin();
        }, 1000);
...
// Sayfa yüklendiğinde olay dinleyicileri oluştur
document.addEventListener('DOMContentLoaded', () => {
  setupAuthEventListeners();
});

/**
 * Kimlik doğrulama için gerekli olay dinleyicilerini ayarla
 */
function setupAuthEventListeners() {
  ...
  // Demo giriş butonu
  const demoLoginBtn = document.querySelector('button[onclick="demoLogin()"]');
  if (demoLoginBtn) {
  ...
  console.log('Kimlik doğrulama olay dinleyicileri kuruldu');
}
 ...
 * @file auth.js
 * @description Firebase kimlik doğrulama işlevleri ve kullanıcı yönetimi.
 * Tüm kimlik doğrulama işlevlerini ve oturum yönetimini sağlar.
 */
...
const authState = {
  ...
  isDemo: false,          // Demo mod durumu
  userRoles: {            // Rol hiyerarşisi
    'admin': 100,
    'manager': 80,
    'supervisor': 60,
    'editor': 40,
 ...
 * @returns {Promise<Object>} - Giriş işlemi sonucu
 */
    ...
    if (typeof firebase !== 'undefined' && firebase.auth) {
      try {
        ...
        if (authError.code === 'auth/network-request-failed' || authError.code === 'auth/internal-error') {
          _enableDemoModeWithLoginPrompt(email);
        }
 ...
 * @private
 */
function _enableDemoModeWithLoginPrompt(email = 'demo@elektrotrack.com') {
 ...
 * Şifre sıfırlama
 * @param {string} email - Kullanıcı e-posta adresi (opsiyonel)
 * @returns {Promise<Object>} - Şifre sıfırlama işlemi sonucu
 */
 ...
 * @returns {Promise<Object>} - Kayıt işlemi sonucu
 */
